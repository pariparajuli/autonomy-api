...
func (m mongoDB) RefreshAccountState(accountNumber string) (bool, error) {
	...
	behaviorScore, behaviorDelta, err := m.NearestGoodBehaviorScore(consts.NEAR_DISTANCE_RANGE, loc)
	if err != nil {
		return false, err
	}
	symptomScore, symptomDelta, err := m.NearestSymptomScore(consts.NEAR_DISTANCE_RANGE, loc)
	if err != nil {
		return false, err
	}
